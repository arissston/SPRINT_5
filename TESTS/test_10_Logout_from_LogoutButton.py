...
def test_successful_logout_from_LK_by_Exit_link_click(driver_login, new_user):
    ...
    driver_login.find_element(By.XPATH, ".//button[text()='Выход']").click()

    WebDriverWait(driver_login, 5).until(
        EC.visibility_of_element_located((By.XPATH, ".//button[contains(text(),'Войти')]")))

    assert driver_login.current_url == "https://stellarburgers.education-services.ru/login"
